 ...
 */
export async function getBinaryPath(packageJsonPath, appBuildInfo, electronVersion, p = process) {
    ...
    if (appBuildInfo.isForge) {
        // Forge case
        const archs = allOfficialArchsForPlatformAndVersion(p.platform, electronVersion);
        const forgeOutDir = appBuildInfo.config?.outDir || 'out';
        outDirs = archs.map((arch) => path.join(path.dirname(packageJsonPath), forgeOutDir, `${appBuildInfo.appName}-${p.platform}-${arch}`));
    }
    else {
        ...
        if (p.platform === 'darwin') {
            ...
            outDirs = archs.map((arch) => path.join(path.dirname(packageJsonPath), builderOutDirMap(arch)[p.platform]));
        }
        else {
            // other platforms have a single output dir which is not dependent on the arch
            outDirs = [
                path.join(path.dirname(packageJsonPath), builderOutDirMap('x64')[p.platform]),
            ];
        }
    }
    const binaryPathMap = {
